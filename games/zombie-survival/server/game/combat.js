...
export function processShot(game, player, msg) {
  ...
  const rpmMod = (player.mods && player.mods.rpm) || 1; // Double Tap
  const interval = shotInterval(w) / rpmMod;
  ...
  if (w.hyperburst) {
    ...
    const bi = burstInterval(w) / rpmMod;
    ...
    if (sinceLast < 0.9 * burstInterval(w) / rpmMod) return;
